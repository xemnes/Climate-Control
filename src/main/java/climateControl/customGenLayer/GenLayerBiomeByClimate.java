    ...
    public int[] getInts(int par1, int par2, int par3, int par4) {
        ...
        for(i1 = 0; i1 < par4; ++i1) {
            for(int j1 = 0; j1 < par3; ++j1) {
                ...
                k1 &= -3841;
                if (k1 > 256 && ClimateControl.testing) {
                    ClimateControl.logger.info(this.parent.toString());
                ...
                if (k1 == Biome.getIdForBiome(Biomes.FROZEN_OCEAN)) {
                    ...
                    for(Iterator var11 = this.partitioners.iterator(); var11.hasNext(); picker = partitioner.partitioned(picker, j1 + par1, i1 + par2)) {
                        partitioner = (DistributionPartitioner)var11.next();
                    }

                    aint1[j1 + i1 * par3] = picker.biome(k1, this.randomCallback);
                }
            }
        }
